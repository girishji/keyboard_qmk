...
static void i2c_reg_write_uint8(uint8_t reg, uint8_t value) {
  ...
  if (status != MSG_OK) {
      if (status == MSG_TIMEOUT) {
          uprintf("i2c transmit timeout\n");
      } else if (status == MSG_RESET) {
          i2cflags_t errf = i2cGetErrors(&I2C_DRIVER);
          uprintf("i2c transmit reset, %lx, %lu\n", errf, errf);
      } else {
          uprintf("i2c transmit error unknown: %lu\n", status);
      }
  }
#endif
  if (status == MSG_TIMEOUT) {
